...
all_files <- reactive({
  ...
  df_cutoff <- which(str_detect(bend_raw_na$'Location', 'Bend Genetics, LLC'))
  
  analytical_report_for_samples <-
    bend_raw_na[1:df_cutoff[1]-1,]
  
  sample_results <-
    bend_raw_na[(which(str_detect(bend_raw_na$'Sample ID', "SAMPLE RESULTS"))+2):(df_cutoff[2]-1),] |>
    rename(
      Method = Location,
