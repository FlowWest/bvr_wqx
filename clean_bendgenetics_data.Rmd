...
First we need to load lookup table that contains project ID lookup (eg.location FC1's corresponding project ID is CS) and unit lookup (eg. Temperature, water - deg C)
```{r}
...
bend_wqx <- bend_full_df |> 
  mutate("Project ID" = "HAB",
        ...
        # Seems like we are only using ELSA analysis not QPCR?
         ...
         "Result Analytical Method Context" = method_lookup[method],
