...
check_depth_range <- function(){
  result <- with(all_files(),
     all(all_files()$Depth10 <= 50 & all_files()$Depth10 >= -10))

  return(list(result = result, description="Checking for 'Depth' column smaller than 50m and bigger than -10m"))
}
...
check_salinity_range <- function() {
  result <- with(all_files(),
                 all(as.numeric(all_files()$Sal) <= 50 &
                   as.numeric(all_files()$Sal) >= 0))
  return(
    list(result = result, description = "Checking for 'Salinity' column smaller than 50 ppt	 and bigger than 0 ppt")
  )
}
...
check_oxygen_saturation_range <- function() {
  result <- with(all_files(),
                 all(as.numeric(all_files()$`DO%`) <= 500 &
...
check_ph_range <- function() {
  result <- with(all_files(),
                 all(as.numeric(all_files()$pH) <= 10 &
                   as.numeric(all_files()$pH) >= 0))
  return(
    list(result = result, description = "Checking for 'pH' column smaller than 10 and bigger than 0")
  )
}
