...
check_oil_range <- function() {
  ...
  if ("Oil & Grease (HEM)" %in% alpha_with_results$ANALYTE){
    result <- with(
      all_files() |> filter(all_files()$`ANALYTE` == "Oil & Grease (HEM)"),
      all(as.numeric(Result) <= 3000 & as.numeric(Result) >= 0))
    
    return(
      list(result = result, description = "Checking for 'Oil & Grease' smaller than 3000 mg/L and bigger than 0")
    )
  }else{
   return(list(result = "False", description = "Checking for Oil & Grease has missing values"))
  }
