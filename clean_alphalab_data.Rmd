...
partial_clean_alpha_lab <- reactive({
  ...
  all_files <- all_files() |>
    ...
    mutate(
      ...
      # use the lubridate package function mdy_hm()to format date in HH:MM
      ...
      # use case_when when there are multiple conditions
      "Result Detection Condition" = case_when(
        Result == "ND" ~ "Below Detection Limit",
        Result == "Absent" ~ "Not Present",
        Result == "Present" ~ "Detected Not Quantified",
        TRUE ~ NA_character_
      ),
      "Result Value" = ifelse(
        Result %in% c("Absent", "ND", "Present"),
        NA_character_,
        Result
      ),
