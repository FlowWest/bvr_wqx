...
all_files <- reactive({
  ...
  raw_alpha_lab <- purrr::map_df(input$alpha_files$datapath, read_excel) |> 
    ...
    dplyr::mutate(SAMPLENAME = ifelse(sample1 %in% project_sites, sample1, sample2)) |>
...
check_unique_values <- function(){
  data <- all_files()
  
  return(
    list(
      description = "Unique water quality analytes collected in the 'ANALYTE' column:",
      result = unique(data$ANALYTE)
    )
  )
}
```
...
# apply_unique_qaqc <- function(unique_list) {
#   
#   purrr::map(unique_list, function(f) {
#     qaqc_result <- f()
#     # emoji <- ifelse(qaqc_result$result, emo::ji("check"), emo::ji("x"))
#     tagList(
#       tags$tr(
#       tags$td(paste(qaqc_result$description), style="padding:5px;"),
#       tags$td(paste(qaqc_result$result), style="padding:5px;")
#       )
#     )
#   })
#   
...
#       border = "1",
#       style="border:2px solid black;width:70%",
#       tags$tr(apply_qaqc(unique_qaqc_funcs)) # <--------------- qaqc get applied here
#     )
#   )
# )
