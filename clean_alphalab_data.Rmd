...
```{r}
fileInput("alpha_files", "Choose AlphaLab Excel File", accept = ".xlsx", multiple = TRUE)
...
```{r}
# fileInput("alpha_folder", "Choose the folder containing AlphaLab Excel File")
# 
# observe({
#   input$alpha_folder
# })
...
# observe({
#   req(input$alpha_files)
#   values <- reactiveValues(df_data = all_files())
# # iterate through the sample name for each row to parse the location
#   # full_df <- 
#   for (row in 1:nrow(values$df_data)) {
#     for (n in 1:length(strsplit(values$df_data$SAMPLENAME, " ")[[row]])) {
#       if (strsplit(values$df_data$SAMPLENAME, " ")[[row]][n] %in% locations) {
#         
#         values$df_data$SAMPLENAME[row] = strsplit(values$df_data$SAMPLENAME, " ")[[row]][n]
#         # values$df_data <- temp
#         print(values$df_data$SAMPLENAME)
#         # print(full_df$SAMPLENAME[row])
#         # print(full_df$SAMPLENAME)
#         # print(strsplit(all_files()$SAMPLENAME, " ")[[row]][n])
#         # print(all_files()$SAMPLENAME[row] == "X") 
#         # all_files <- all_files() |>
#           # mutate("Location" = strsplit(raw_alpha_lab$SAMPLENAME, " ")[[row]][n])
#       }
#     }
#   }
# })
